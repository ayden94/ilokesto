...
Every value passed to `.use()` must be registered with `definePipeableMiddleware()`. Give each middleware an `id`; use metadata to declare `adds`, `requires`, `before`, `after`, `conflicts`, and the optional `duplicate` policy. Capabilities added by an earlier outer middleware are available to later inner middleware. A relationship to middleware absent from the chain is ignored, but present relationships, conflicts, duplicate IDs, and cycles are rejected without reordering.

## Configuration errors

`PipeConfigurationError` exposes the failing middleware `id`, related `ids`, and a code: `DUPLICATE_CAPABILITY`, `DUPLICATE_MIDDLEWARE`, `INVALID_METADATA`, `INVALID_MIDDLEWARE_RESULT`, `INVALID_STORE_INPUT`, `MISSING_CAPABILITY`, `MIDDLEWARE_CONFLICT`, `MIDDLEWARE_CYCLE`, or `MIDDLEWARE_ORDER`.
