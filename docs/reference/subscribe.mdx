...
The store does not catch listener errors. If a selector, a listener, or an equality function throws, the throw propagates synchronously out of the `setState()` call and any later listeners (selector or plain) that would have run in the same notification cycle are skipped. The selector, listener, and equality function all run synchronously inside the notification cycle. Keep them small or catch expected errors inside the listener.
